...
/**
 * Performance Metrics Collection for Enhanced Moshi H2 JSON Processing.
 *
 ...
 * processing without requiring WildFly dependencies.
 *
 ...
 */
public class JsonProcessingMetrics {
    private static final Log log = LogFactory.getLog(JsonProcessingMetrics.class);
    ...
    public void recordProcessingStart(String requestId, long payloadSize, boolean isAsync) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Started Moshi H2 processing request: " + requestId + " (" + formatBytes(payloadSize) +
                      ", async=" + isAsync + ")");
        }
    }
    ...
    public void recordProcessingComplete(String requestId, long payloadSize, long processingTimeMs) {
        ...
        if (request != null) {
            ...
            if (processingTimeMs > 10000) { // 10s threshold
                ...
                log.warn("Slow Moshi H2 processing detected: " + requestId + " took " + processingTimeMs + "ms");
            }
            if (processingTimeMs > 15000) { // 15s threshold (production issue analysis)
                verySlowRequestCount.incrementAndGet();
                log.error("Very slow Moshi H2 processing detected: " + requestId + " took " + processingTimeMs + "ms - " +
                         "This matches the production 12-18s issue pattern");
            }

            if (log.isDebugEnabled()) {
                log.debug("Completed Moshi H2 processing request: " + requestId + " in " + processingTimeMs + "ms");
            }
        } else {
    ...
    public void recordProcessingError(String requestId, Exception error, long processingTimeMs) {
        ...
        log.warn("Moshi H2 processing error for request: " + requestId + " after " + processingTimeMs + "ms: " +
                 error.getMessage());
    }
    ...
    public void recordLargeArrayProcessing(String fieldName, int arraySize, long processingTimeMs) {
        ...
        if (arraySize > 10000) {
            log.info("Processed large array field '" + fieldName + "': " + arraySize +
                     " items in " + processingTimeMs + "ms (Moshi H2 optimization)");
        }
    }
    ...
    public void recordMemoryOptimization(String requestId, String optimizationType) {
        if (log.isDebugEnabled()) {
            log.debug("Memory optimization applied for Moshi H2 request: " + requestId +
    ...
    public void recordStreamingActivity(String requestId, String activityType, long durationMs) {
        if (log.isDebugEnabled()) {
            log.debug("Moshi H2 streaming activity: " + requestId + " - " + activityType + " (" + durationMs + "ms)");
        }
    }
    ...
    public String getOptimizationRecommendations() {
        ...
        recommendations.append("Enhanced Moshi H2 JSON Processing Optimization Recommendations:\n");
    ...
    public void resetStatistics() {
        ...
        log.info("Moshi H2 JsonProcessingMetrics statistics reset");
    }
    ...
    public static class Statistics {
        ...
        public String toString() {
            return String.format(
                "MoshiH2Metrics{requests=%d, bytes=%s, avgTime=%.1fms, " +
